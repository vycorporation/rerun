...
| Graph hierarchy and named graph registry | partial | `graph-hierarchy-tracker.md` | `#130`, `#131`, `#132`, `#142`, `#144`, `#146` when merged |
...
1. Multiple named graphs and subgraph navigation.
   ...
   - Issue-backed slices: `#130` graph registry metadata, `#131` current graph path and graph-local inspection, `#132` focused graph hierarchy tracker, `#142` graph container metadata, `#144` typed graph container boundaries, `#146` boundary anchor mappings when merged.
   - Later likely slices: graph-local unique node names, selected graph navigation, selected-node collapse, and visible invalid/cyclic diagnostics.
