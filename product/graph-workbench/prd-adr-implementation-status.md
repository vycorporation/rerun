...
| Graph hierarchy and named graph registry | partial | `graph-hierarchy-tracker.md` | `#130`, `#131`, `#132`, `#142`, `#144`, `#146`, `#148`, `#164`, `#166`, `#168`, `#170` |
...
1. Multiple named graphs and subgraph navigation.
   ...
   - Issue-backed slices: `#130` graph registry metadata, `#131` current graph path and graph-local inspection, `#132` focused graph hierarchy tracker, `#142` graph container metadata, `#144` typed graph container boundaries, `#146` boundary anchor mappings, `#148` collapse manifests, `#164` graph-local node name policy, `#166` selected graph navigation model actions, `#168` selected graph storage layout scope, and `#170` physical graph container collapse storage.
   - Later likely slices: visible invalid/cyclic diagnostics and UI wiring for graph-container navigation/collapse actions.
